...
const ActiveApplicationsSection: React.FC = () => {
  ...
  async function getUsers() {
    ...
    data.forEach((user: UserModel) => {
      if (
        user.applications &&
        !state_tracks.find(curr => curr._id === user.applications!![0]._id)
      ) {
  ...
  async function getPosition() {
    const viewdPosition = await getPositionById(positionId ? positionId : "");
  ...
  return (
    <ActiveApplicationsSectionStyled>
      ...
      >
        ...
        <Grid item container>
          <h3 className="activeActiveCandidateTitle">Active Candidates:</h3>
        </Grid>
